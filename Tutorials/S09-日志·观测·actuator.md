...
## 4. 工程实录：真实问题与解决——org.hibernate 抬到 DEBUG 后日志失血，再收回到 INFO 的行数对账

**问题**：S03 那节让我们把 `org.hibernate.SQL` 调到 DEBUG 看 SQL——但"关一次"与"开着不管"是两回事。Hibernate 在 DEBUG 下不只是两条 INSERT：**建表、方言、注册、参数绑定、结果追踪**全是 DEBUG 事件。观察一个教学实验：同一份 demo-todo、同一个 POST 各打一次，两个实例的文件日志只差一个参数。

### 现场复现

```bash
cd /tmp/opencode
# A：故意把 org.hibernate 调到 DEBUG（可以看到 SQL，也会看到 723 行其他噪音）
nohup java -jar ~/Projects/javaweb/backend/demo-todo/target/demo-todo-1.0.0.jar \
    --server.port=8091 --spring.datasource.url='jdbc:h2:mem:logdbg' \
    --logging.file.name=/tmp/opencode/log9_dbg.log \
    --logging.level.org.hibernate=DEBUG > /dev/null 2>&1 &
# B：同理换端口，把 hibernate 级别"强改"回 INFO
nohup java -jar ~/Projects/javaweb/backend/demo-todo/target/demo-todo-1.0.0.jar \
    --server.port=8092 --spring.datasource.url='jdbc:h2:mem:loginf' \
    --logging.file.name=/tmp/opencode/log9_inf.log \
    --logging.level.org.hibernate=INFO > /dev/null 2>&1 &
sleep 25
curl -s -X POST 127.0.0.1:8091/api/tasks -H 'Content-Type: application/json' -d '{"title":"对照A"}' > /dev/null
curl -s -X POST 127.0.0.1:8092/api/tasks -H 'Content-Type: application/json' -d '{"title":"对照B"}' > /dev/null
```

**实测输出（同一动作、同一份日志格式，级别开关 = 23 倍噪声差）：**

```
$ wc -l log9_dbg.log log9_inf.log
     725 log9_dbg.log      ← org.hibernate=DEBUG：含建表/方言/HQL 翻译等全部内幕
      32 log9_inf.log      ← org.hibernate=INFO：只剩人话级大事
```

DEBUG 版内容抽样（`grep org.hibernate.SQL` 两行，也证明了"建表也是它写的"）：

```
2026-09-15T10:00:57.696+08:00 DEBUG ... [main] org.hibernate.SQL : create table task (id bigint generated by default as identity, done boolean not null, title varchar(255) not null, primary key (id))
2026-09-15T10:01:19.936+08:00 DEBUG ... [http-nio-8091-exec-1] org.hibernate.SQL : insert into task (done,title,id) values (?,?,default)
```

INFO 版第一屏（同样的行有、但更冷清；`head -3 log9_inf.log`）：

```
2026-09-15T10:00:55.515+08:00  INFO 135942 --- [demo-todo] [main] com.javaweb.todo.TodoApp : Starting TodoApp v1.0.0 using Java 26.0.2.1 with PID 135942 ...
2026-09-15T10:00:55.516+08:00  INFO 135942 --- [demo-todo] [main] com.javaweb.todo.TodoApp : No active profile set, falling back to 1 default profile: "default"
2026-09-15T10:00:56.115+08:00  INFO 135942 --- [demo-todo] [main] .s.d.r.c.RepositoryConfigurationDelegate : Bootstrapping Spring Data JPA repositories in DEFAULT mode.
```
...
| 何时用哪档 | 症状/目的 | 命令 |
|---|---|---|
| 排 SQL 问题（N+1、慢查） | 数 SELECT 条数 | `--logging.level.org.hibernate.SQL=DEBUG` |
| 业务日志堆成山想压噪声 | 回 INFO | `--logging.level.org.hibernate=INFO`（本站实测 -96%） |
| 追 Hibernate 参数绑定 | 看每个 ? 真值 | `--logging.level.org.hibernate.orm.jdbc.bind=TRACE` |

**刻度**：级别开关不是"开/关 semantics 的开关"，是**区域广播音量旋钮**。生产语义"看起来不重要"的 DEBUG 不代表无害：**磁盘、检索成本、日志里泄`绑定参数`的隐私风险**（用户名/金额都会被 `bind` 拍进去）。上 DEBUG 排障后要"回收"——一条命令的事，却常常被忘。
